...
import Reports from '../components/Row';
...
function SupervisorProfile(props) {
  ...
  useEffect(() => { //this will be executed by react but only under certain circumstances
    ...
    axios.get(`${API_URL}/get-employee-skills/${localStorage.getItem('staffnumber')}`)
      .then(response => {
  ...
  useEffect(() => { //this will be executed by react but only under certain circumstances
    ...
    axios.get(`${API_URL}/get-subordinates/${localStorage.getItem('staffnumber')}`)
      .then(response => {
